...
class AutoAttacker:
    ...
    def _execute_attack_sequence(self) -> bool:
        ...
        try:
            ...
            time.sleep(1)  # Wait for attack screen
            ...
            battle_wait_time = 150  # 3 minutes
    ...
    def _find_good_loot_target(self) -> bool:
        ...
        if 'confirm_attack' not in coords:
            self.logger.error("confirm_attack button not mapped")
            return False
        ...
        while search_attempts < max_attempts and self.is_running:
            ...
            self.logger.info(f"2️⃣ Clicking find_a_match at ({find_coord['x']}, {find_coord['y']})")
            pyautogui.click(find_coord['x'], find_coord['y'])
            time.sleep(1)  # Wait for confirm screen

            confirm_coord = coords['confirm_attack']
            self.logger.info(f"3️⃣ Clicking confirm_attack at ({confirm_coord['x']}, {confirm_coord['y']}) - Attempt {search_attempts}/{max_attempts}")
    ...
    def configure_buttons(self) -> Dict[str, str]:
        ...
        return {
            ...
            'confirm_attack': 'Confirm attack button after matchmaking prompt',
